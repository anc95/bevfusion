...
def main() -> None:
    ...
    parser.add_argument("pc_path", metavar="FILE")
    ...
    pc = np.fromfile(args.pc_path, dtype=np.float32).reshape(-1, 5)
    pc[:, 4] = 0
    
    data = list(dataflow)[0]

    data['points'] = DataContainer([[torch.from_numpy(pc)]], stack=True)
